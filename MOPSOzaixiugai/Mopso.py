...
class Mopso:
    def __init__(self,particals,max_,min_,thresh,mesh_div=10):
        ...
        self.max_v = np.inf * np.ones(len(max_), )  # 速度上限,速度不存在上下限，因此设置很大
        self.min_v = -1000 * np.ones(len(min_), )  # 速度下限

        self.plot_ = plot.Plot_pareto()

    def evaluation_fitness(self):
        self.fitness_ = P_objective.P_objective("value", "VMD", 2, self.in_)
        print('fitness')
        print(self.fitness_)
    def initialize(self):
