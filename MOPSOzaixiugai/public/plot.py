...
class Plot_pareto:
    ...
    def show(self,in_,fitness_,archive_in,archive_fitness,i):
        ...
        print('%%%%%%%%%%%%%%%%%%%%%%%%% fitness_个数 %%%%%%%%%%%%%%%%%%%%%%%%%')
        print(fitness_.shape,archive_fitness.shape)
        ax3.scatter(fitness_[:,0],fitness_[:,1],s=10, c='c', marker=".")
        ax3.scatter(archive_fitness[:,0],archive_fitness[:,1],s=90, c='red', marker="*",alpha = 1.0)
        plt.title('第' + str(i + 1) + '次迭代')
        plt.show()
