...
def update_v(v_,v_min,v_max,in_,in_pbest,in_gbest):
    ...
    print('v_',v_.shape,type(v_),'r1',r1.shape,type(r1),'r2',r2.shape,type(r2))
    print('in_',in_,in_.shape,type(in_),'in_pbest',in_pbest,in_pbest.shape,type(in_pbest),'in_gbest',in_gbest,in_gbest.shape,type(in_gbest))
    print('w*v_',w*v_)
    v_temp = w*v_ + r1*(in_pbest-in_) + r2*(in_gbest-in_)
    print('v_temp111', v_temp)
    #速度边界处理ֵ
...
def update_in(in_,v_,in_min,in_max):
    ...
    print('uodate_in', in_temp, type(in_temp))
    in_temp = np.rint(in_temp)
    in_temp = in_temp.astype(np.int)
    print('uodate_in11',in_temp,type(in_temp))
    return in_temp
